...
const personaSchema = z.object({
  name: z
    .string()
    .describe(
      'A descriptive name for the persona (e.g., "Tech-Savvy Millennials")'
    ),
  ...
  demographics: z
    .object({
      ...
      location: z
        .string()
        .describe('Primary location type (e.g., "Urban areas", "Suburban")'),
      education: z
        .string()
        .describe(
      ...
      occupation: z
        .string()
        .describe(
          'Common job types (e.g., "Software developers", "Marketing professionals")'
        ),
    })
  ...
  behaviors: z
    .object({
      ...
      purchasingBehavior: z
        .array(z.string())
        .describe("How they make purchasing decisions"),
  ...
  preferences: z
    .object({
      contentTypes: z
        .array(z.string())
        .describe("Types of content they prefer"),
      channels: z
        .array(z.string())
        .describe("Marketing channels they respond to"),
      messagingTone: z.string().describe("Preferred tone of communication"),
      valuePropositions: z
        .array(z.string())
        .describe("What value propositions resonate with them"),
      brandAttributes: z
        .array(z.string())
        .describe("Brand characteristics they prefer"),
    })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const totalRevenue = platformUsers.reduce(
      (sum, user) => sum + (user.monthlySpendUsd || 0),
      0
    );
    ...
    const result = await generateObject({
      ...
      prompt: `
        ...
        TOP INDUSTRIES:
        ${analyticsData.topIndustries
          .map(
            (item) =>
              `- ${item.industry}: ${item.count} users (${item.percentage}%)`
          )
          .join("\n")}
        
        TOP AGE GROUPS:
        ${analyticsData.topAgeGroups
          .map(
            (item) => `- ${item.age}: ${item.count} users (${item.percentage}%)`
          )
          .join("\n")}
        ...
        ${analyticsData.topLocations
          .map(
            (item) =>
              `- ${item.location}: ${item.count} users (${item.percentage}%)`
          )
          .join("\n")}
        
        TOP JOB TITLES:
        ${analyticsData.topJobTitles
          .map(
            (item) =>
              `- ${item.jobTitle}: ${item.count} users (${item.percentage}%)`
          )
          .join("\n")}
        
        RECENT USER SAMPLE:
        ${analyticsData.recentSignups
          .slice(0, 5)
          .map(
            (user) =>
              `- ${user.jobTitle || "Unknown"} in ${
                user.industry || "Unknown"
              }, ${user.location || "Unknown"}, Age: ${
                user.age || "Unknown"
              }, Spend: $${user.monthlySpend || 0}/month`
          )
          .join("\n")}
