...
const campaignSchema = z.object({
  name: z.string().describe('A compelling campaign name (e.g., "Tech Professional Engagement Drive")'),
  description: z.string().describe('A detailed description of the campaign objectives and approach'),
  strategy: z.string().describe('A comprehensive marketing strategy including channels, messaging, timeline, and tactics'),
  budget: z.number().optional().describe('Suggested budget in USD for this campaign'),
  startDate: z.string().optional().describe('Suggested start date in ISO format'),
  ...
  try {
    ...
    const savedCampaign = await prisma.campaign.create({
      data: {
        ...
        startDate: result.object.startDate ? new Date(result.object.startDate) : null,
